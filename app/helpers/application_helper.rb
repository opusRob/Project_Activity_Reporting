module ApplicationHelper
	...
	include ApplicationMethodsHelper
	...
	def link_to_bootstrap_glyphicon_unless(condition, path, size, type, title, glyphicons, options = {}, link_html_options = {})
		# Available options:  text_start, text_end.
		link_to_unless condition, path, class: "btn btn-#{size} btn-#{type}", title: title, "aria-label" => title, method: options[:method], data: options[:data], html_options: link_html_options do
			link_inner_html = options[:text_start].to_s
			glyphicons.each do |glyphicon|
				link_inner_html += '<span class="glyphicon glyphicon-' + glyphicon + '" aria-hidden="true"></span>'
			end
	...
	def button_tag_bootstrap_glyphicon(button_type, name, size, type, title, glyphicons, options = {}, button_html_options = {})
		# Available options:  text_start, text_end.
		options[:disabled] = options.has_key?(:disabled) ? options.has_key?(:disabled) : false
		button_tag type: button_type, name: name, class: "btn btn-#{size} btn-#{type} #{options[:class]}", disabled: options[:disabled], title: title, "aria-label" => title, onclick: options[:onclick], html_options: button_html_options do
	...
	def button_to_bootstrap_glyphicon(action, button_type, name, size, type, title, glyphicons, options = {}, button_html_options = {})
		# Available options:  text_start, text_end.
		options[:disabled] = options.has_key?(:disabled) ? options.has_key?(:disabled) : false
		button_to action, type: button_type, name: name, class: "btn btn-#{size} btn-#{type} #{options[:class]}", disabled: options[:disabled], title: title, "aria-label" => title, html_options: button_html_options do
			button_inner_html = options[:text_start].to_s
			glyphicons.each do |glyphicon|
				button_inner_html += '<span class="glyphicon glyphicon-' + glyphicon + '" aria-hidden="true"></span>'
			end
